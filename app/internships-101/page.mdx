...
<div className="bg-card rounded-lg p-4">
  <p className="text-sm m-0 p-0">While I won't be covering the basics here, you should still focus on creating a {Array.from("professional CV, writing tailored cover letters, practising LeetCode, building projects, attending hackathons, preparing answers for behavioural questions, gathering experience, networking, doing mock interviews").map((char, index, array) => ( <span key={index} style={{fontSize: `${Math.max(14 - (index * 0.05), 0)}px`}}>{char}</span>))} ...</p>
</div>
...
As a CTO of a *very* large company once awkwardly said in front of ~800 attendees at a hackathon:
...
(Looking back, this was probably one of the most uncomfortable things I've heard someone in their 50s say at a hackathon, but it is true.)
...
Even if you're aiming to be a back-end engineer, I would suggest having a website is a good idea. You'll show your willingness to go the extra mile and out of your comfort zone.
